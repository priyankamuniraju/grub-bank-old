...
public class GrubBankCRUDController {
  ...
  /**
   * @param recipeSearchCriteria the object which specifies the search criteria for searching
   *     recipes
   * @return
   */
  @PostMapping("/search")
  ResponseEntity<GrubBankResponseBody<Object>> searchByCriteria(
    ...
    try {
      ...
      logger.error(
          String.format(
              NO_RECIPES_FOUND_WITH_MESSAGE, invalidRecipeSearchCriteriaException.getMessage()));
      return new ResponseEntity<>(
          new GrubBankResponseBody<>(
              String.format(
                  NO_RECIPES_FOUND_WITH_MESSAGE, invalidRecipeSearchCriteriaException.getMessage()),
